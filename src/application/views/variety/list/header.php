...
<fieldset class="search-fieldset">
	...
	<?php if (isset ( $options )) : ?>
		...
		<?php for($i = 0; $i < count ( $options ); $i ++):?>
       	...
       	<?php else:?>
       	<li>
       	<?php echo ucwords(clean_string($keys [$i])); ?>:&nbsp;<strong><?php echo ucwords(clean_string($values [$i]));?></strong>
</li>
       <?php	endif;?>
...
echo create_button_bar(array(array("text"=>"Refine Search","class"=>array("button","search","dialog","refine"),"href"=>site_url("variety/search?refine=1"))));?>
