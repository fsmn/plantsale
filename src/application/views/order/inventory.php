...
if ($orders) :
	...
	<table class="list inventory hideable-columns">
		...
		foreach ($orders as $order) :
			...
			$thursday_total += $order->count_thursday;
			$friday_total += $order->count_friday;
			$saturday_total += $order->count_saturday;
			$midsale_total += $order->count_midsale;
			$flat_cost_total += $order->flat_cost * ($order->count_presale + $order->count_midsale + $order->count_thursday + $order->count_friday + $order->count_saturday);
			...
			<tr class="<?php echo implode(" ", $row_classes); ?>"
				...
				<td class="order-remainder_friday field" style="width: 31px;">
					<?php echo live_field("remainder_friday", $order->remainder_friday, "order", $order->id, [
							"envelope" => "span",
							"sizse" => 31,
					]); ?>
				</td>
				<td class="order-sellout_friday field" style="width: 31px;">
					<?php echo live_field("sellout_friday", $order->sellout_friday, "order", $order->id, [
							"envelope" => "span",
							"sizse" => 31,
					]); ?>
				</td>
				...
				<td class="order-received_midsale field">
					<?php echo live_field("received_midsale", $order->received_midsale, "order", $order->id, ["envelope" => "span"]); ?>
				</td>
				...
				<td class="order-sellout_saturday field" style="width: 31px;">
					<?php echo live_field("sellout_saturday", $order->sellout_saturday, "order", $order->id, [
							"envelope" => "span",
							"sizse" => 31,
					]); ?>
				</td>
		...
		<tr>
			...
			<?php if ($latest_year == 2021): ?>
				<td><?php echo number_format($thursday_total); ?></td>
			...
			<?php endif; ?>
			<td colspan=5></td>
			<td><?php echo number_format($presale_total + $midsale_total + $thursday_total + $friday_total + $saturday_total); ?></td>
			<td></td>
			<td></td>
