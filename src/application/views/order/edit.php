...
<form name="order-edit" id="order-edit" class="no-wrap" action="<?php print site_url('order/' . $action); ?>" method="post">
	...
	<?php endif; ?>
	<input type="hidden" name="id" value="<?php print get_value($order, 'id'); ?>" />
	...
	<?php if ($action == 'update') : ?>
		...
		<div class="order-grower field">
			...
			<input type="text" name="grower_id" value="<?php print get_value($order, 'grower_id'); ?>" />
		</div>
		<div class="order-catalog_number field">
			<label for="catalog_number">Catalog Number
				<input type="text" name="catalog_number" value="<?php print get_value($order, 'catalog_number'); ?>" /></label>
		</div>
		<div class="order-flat_size field">
			<label for="flat_size">Flat Size:
				<input type="text" name="flat_size" class="size" value="<?php print get_value($order, 'flat_size'); ?>" required autocomplete="off" /></label>
		</div>
		<div class="order-flat_cost field">
			<label for="flat_cost">Flat Cost:&nbsp;$
				<input type="text" name="flat_cost" class="cost" value="<?php print get_value($order, 'flat_cost'); ?>" required autocomplete="off" /></label>
		</div>
		<div class="order-plant_cost field">
			<label for="plant_cost">Plant Cost:&nbsp;$
				<input type="text" class="cost" name="plant_cost" value="<?php print get_value($order, 'plant_cost'); ?>" autocomplete="off" required /></label>
		</div>
		<div class="order-price field">
			<label for="price">Price:&nbsp;$
				<input type="text" name="price" value="<?php print get_value($order, 'price'); ?>" required autocomplete="off" /></label>
		</div>
		</div>
		<div class="column last">
			...
			<div class="order-grower_code field">
				...
				<input type="text" name="grower_code" value="<?php print get_value($order, 'grower_code'); ?>" />
			</div>
			<div class="order-flat_area field">
				<label for="flat_area">Flat Area (Sq Ft):&nbsp;</label>
				<input type="text" name="flat_area" value="<?php print get_value($order, 'flat_area', 2); ?>" size="10" />
			</div>
			<div class="order-tiers field">
				<label for="tiers">Tiers:&nbsp;</label>
				<input type="text" name="tiers" value="<?php print get_value($order, 'tiers', 3); ?>" size="10" />
			</div>
		</div>
		<div class="group">
				...
				<?php foreach ($days as $day) : ?>
					<div class="order-count_presale field">
						<label for="count_<?php print $day; ?>"><?php print ucfirst($day); ?>
							Count:&nbsp;
							<input type="number" class="count" name="count_<?php print $day; ?>" value="<?php print get_value($order, 'count_' . $day); ?>" autocomplete="off" /></label>
					</div>
				<?php endforeach; ?>
			</div>
			<div class="column last">
					...
					<?php foreach ($days as $day) : ?>
						...
						<div class="order-<?php print $field_name; ?> field">
							<label for="<?php print $field_name; ?>"><?php print ucfirst($day); ?>
								Received:&nbsp;</label>
							<input type="text" style="<?php print $crop_failure ? 'background-color:#FFB3B3' : ''; ?>" name="<?php print $field_name; ?>" value="<?php print get_value($order, $field_name); ?>" autocomplete="off" />
						</div>
					<?php endforeach; ?>
					<?php if (get_value($order, 'year') != 2020) : ?>
						<div class="order-sellout_friday field">
							<label for="sellout_friday">Sellout
								Friday:&nbsp;</label> <input type="text" name="sellout_friday" value="<?php print get_value($order, 'sellout_friday'); ?>" size="6" />
						</div>
						<div class="order-remainder_friday field">
							<label for="remainder_friday">Remainder
								Friday:&nbsp;</label> <input type="text" name="remainder_friday" value="<?php print get_value($order, 'remainder_friday'); ?>" size="3" />
						</div>
						<div class="order-sellout_saturday field">
							<label for="sellout_saturday">Sellout
								Saturday:&nbsp;</label> <input type="text" name="sellout_saturday" value="<?php print get_value($order, 'sellout_saturday'); ?>" size="6" />
						</div>
						<div class="order-remainder_saturday field">
							<label for="remainder_saturday">Remainder
								Saturday:&nbsp;</label>
							<input type="text" name="remainder_saturday" value="<?php print get_value($order, 'remainder_saturday'); ?>" size="3" />
						</div>
						<div class="order-remainder_sunday field">
							<label for="remainder_sunday">Remainder
								Sunday:&nbsp;</label>
							<input type="text" name="remainder_sunday" value="<?php print get_value($order, 'remainder_sunday'); ?>" size="3" />
						</div>
						<div class="order-count_dead field">
							<label for="count_dead">Dead Count:&nbsp;</label>
							<input type="text" name="count_dead" value="<?php print get_value($order, 'count_dead'); ?>" size="3" />
						</div>
					<?php endif; ?>
			</div>
		<?php endif;
		...
		<div style="clear:both">
			<input type="submit" value="<?php print ucfirst($action); ?>" class="button <?php print $action; ?>" />
			<?php if ($action == "update") : ?>
