...
class Variety_Model extends CI_Model
{
    ...
    function get ($id)
    {
        ...
        $this->db->select(
                "variety.*, variety.id as id, variety.common_id as common_id, common.name as common_name, common.genus,  common.category, common.description, common.sunlight, common.extended_description, common.other_names");
    ...
    function get_by_name ($name)
    {
        ...
        $this->db->select(
                "variety.*, variety.id as id, variety.common_id as common_id, common.name as common_name, common.genus,  common.category, common.description");
    ...
    {
        $query = sprintf(
                "SELECT count(`the_table`.`the_count`) as new_varieties from (SELECT count(`variety_id`) as `the_count`, `order`.`year` from variety join `order` on `variety`.`id` = `order`.`variety_id` GROUP BY `order`.`variety_id` ORDER BY `order`.`year` DESC) as `the_table` WHERE `year` = %s",
                $year);
