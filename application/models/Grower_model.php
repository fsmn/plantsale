...
class grower_model extends MY_Model
{
    ...
    {
/*         $query = sprintf(
                "SELECT sum(`o`.`total`) as `total`, `grower`.* FROM (SELECT `grower_id`, (IFNULL(`count_presale`,0) + IFNULL(`count_midsale`,0)) * `flat_cost` as `total` FROM (`orders`) WHERE `year` = '%s' AND `orders`.`grower_id` = '%s' )  as `o` LEFT JOIN `grower` on `grower`.`id` = `o`.`grower_id` GROUP BY `o`.`grower_id`",
                $year, $id); */
        ...
        $query = sprintf( "SELECT sum(`o`.`total`) as `total`, `grower`.*,`users`.`first_name`,`users`.`last_name`,
                ...
                FROM (SELECT `grower_id`, (IFNULL(`count_presale`,0) + IFNULL(`count_midsale`,0)) * `flat_cost` as `total` FROM (`orders`)
