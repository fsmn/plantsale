...
class MY_Controller extends CI_Controller
{
	...
	function __construct()
	{
		...
		if (!$this->ion_auth->logged_in()) {
			...
			define('IS_EDITOR', $this->ion_auth->in_group([
				1,
				2,
			]));
			define('IS_ADMIN', $this->ion_auth->in_group([
				1,
			]));
			define('IS_INVENTORY', $this->ion_auth->in_group([
				4,
			]));
	...
	function set_options(&$options, $keys = [])
	{
		foreach ($keys as $key) {
