	...
	<table class="list catalog hideable-columns">
		<thead>
			<tr>
				...
				<th class="hide-column" title="Include or exclude these items from the flat totals calculator on the home page">
		...
		<tbody>
			...
			?>
				<tr class="<?php print implode(' ', $row_classes); ?>" id="order_<?php print $order->id; ?>" data-order-id="<?php print $order->id; ?>">
					<td class="no-wrap">
						<?php if (IS_ADMIN) : ?>
							<span class="omit-row omit button" id="omit-order_<?php print $order->id; ?>">Omit</span>
						<?php endif; ?>
						<?php if (IS_EDITOR) : ?>
					...
					<?php if (!empty($show_names)) : ?>
						<td>
							<a href="<?php print site_url(sprintf('common/search?find=1&genus=%s', $order->genus)); ?>" title="View all <?php print $order->genus; ?>"><?php print $order->genus; ?></a>
						</td>
						<td><?php print $order->species; ?></td>
						<td>
							<a href="<?php print site_url('common/view/' . $order->common_id); ?>" title="View the details for <?php print $order->name; ?>"><?php print $order->name; ?></a>
						</td>
						<td>
							<a style="font-weight: bold" href="<?php print site_url('variety/view/' . $order->variety_id); ?>" title="View the details for <?php print $order->variety; ?>"><?php print $order->variety; ?></a>
						</td>
					<?php endif; ?>
					...
					<td class="order-flat_size field cost-field" id="flat_size">
						<span id="edit-flat-size_<?php print $order->id; ?>" class="edit-cost"><?php print $order->flat_size; ?></span>
					</td>
					<td class="order-flat_cost field cost-field no-wrap" id="flat_cost">
						$
						<span id="edit-flat-cost_<?php print $order->id; ?>" class="edit-cost"><?php print number_format($order->flat_cost, 2); ?></span>
					</td>
					<td class="order-plant_cost field cost-field no-wrap" id="plant_cost">
						$
						<span id="edit-plant-cost_<?php print $order->id; ?>" class="edit-cost"><?php print number_format($order->plant_cost, 2); ?></span>
					</td>
