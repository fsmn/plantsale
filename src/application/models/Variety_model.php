...
class Variety_Model extends MY_Model {
	...
	function get_flat_totals($year) {
		...
		$exclusions = $this->settings->get_by_key('flat_total_exclude');
		$this->db->from('variety');
		$this->db->join('orders', 'variety.id=orders.variety_id');
		$this->db->join('common', 'common.id=variety.common_id');
		$this->db->join('category', 'common.category_id = category.id', 'LEFT');
		...
		/*foreach($exclusions as $exclusion){
			$this->db->where('NOT (`orders`.`pot_size` = \'' . $exclusion->value .  '\' AND `category_id` = 7)', NULL, FALSE);
		}*/
		...
		$this->db->where('orders.flat_exclude IS NOT','NULL', NULL,FALSE);
...
$this->db->where('`common`.`subcategory_id` IS NOT','NULL',NULL,FALSE);
	 ...
	 * $this->db->query ( $query );
	 * $query = sprintf ( "update `variety`, `orders` SET `needs_bag` = 1 WHERE
	 * `orders`.`year` = '%s' AND `variety`.`id` = `orders`.`variety_id` and
	 * (`orders`.`pot_size` LIKE '%s' OR
	 * `orders`.`pot_size` LIKE '%s'  OR
	 * `orders`.`pot_size` LIKE '%s' OR
	 * `orders`.`pot_size` LIKE '%s')", get_current_year (), "%bareroot%",
	 * "%bulb%", "%bulb%", "%pound%" );
	 * $this->db->query ( $query );
	 * }
	 */
