...
class Index extends MY_Controller {
	...
	function set_year() {
		...
		burn_cookie('sale_year');
		$this->session->set_userdata('sale_year', $year);
		bake_cookie('sale_year', $year);
		$this->set_sale_year($year);
	...
	function quark() {
		...
		if ($category_id = $this->input->get('category_id') && $subcategory_id = $this->input->get('subcategory_id')) {
			$commons = $this->common->get_for_year($this->get_sale_year(), $category_id, $subcategory_id);
		...
		elseif ($category_id = $this->input->get('category_id')) {
			$commons = $this->common->get_for_year($this->get_sale_year(), $category_id);
			$category = $this->category->get($category_id)->category;
		}
		else {
			$commons = $this->common->get_for_year($this->get_sale_year());
		}
		foreach ($commons as $common) {
			$common->varieties = $this->variety->get_for_quark($common->id, $this->get_sale_year());
