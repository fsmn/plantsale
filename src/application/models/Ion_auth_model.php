 ...
 *   customization.  This is basically what Redux Auth 2 should be. Original
 *   Author name has been kept but that does not mean that the method has not
 *   been modified.
 *
...
class Ion_auth_model extends CI_Model {
	...
	public function __construct() {
		...
		if ($this->hash_method == 'bcrypt') {
			if ($this->random_rounds) {
				...
				$params = ['rounds' => $this->default_rounds];
			}
	...
	/**
	 * Generates a random salt value for forgotten passwords or any other keys.
	 * Uses SHA1.
	 *
	 * @return void
	 * @author Mathew
	 ...
	 *
	 * Salt generation code taken from
	...
	public function forgotten_password_complete($code, $salt = FALSE) {
		...
		$profile = $this->where('forgotten_password_code', $code)
			->users()
			->row(); //pass the code to profile
	...
	public function register($username, $password, $email, $additional_data = [], $groups = []) {
		...
		$query = $this->where('name', $this->config->item('default_group', 'ion_auth'))
			->group();
	 ...
	 * Based on code from Tank Auth, by Ilya Konyukhov
	...
	/**
	 * Get the time of the last time a login attempt occured from given
	...
	public function login_remembered_user() {
		...
		if (!cookie($this->config->item('identity_cookie_name', 'ion_auth'))
			|| !cookie($this->config->item('remember_cookie_name', 'ion_auth'))
			|| !$this->identity_check(cookie($this->config->item('identity_cookie_name', 'ion_auth')))) {
			$this->trigger_events([
		...
		$query = $this->db->select($this->identity_column . ', id, username, email, last_login')
			->where($this->identity_column, cookie($this->config->item('identity_cookie_name', 'ion_auth')))
			->where('remember_code', cookie($this->config->item('remember_cookie_name', 'ion_auth')))
			->limit(1)
			->order_by('id', 'desc')
			->get($this->tables['users']);
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = []) {
		...
		$existing_group = $this->db->get_where($this->tables['groups'], ['name' => $group_name])
			->num_rows();
		if ($existing_group !== 0) {
