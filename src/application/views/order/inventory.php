...
?>
	...
	<table class="list inventory hideable-columns">
		...
		<tbody>
			...
			?>
				<tr class="<?php print implode(" ", $row_classes); ?>" id="order_<?php print $order->id; ?>" title="<?php print $row_title; ?>">
					<td class="no-wrap">
						<?php if (IS_ADMIN) : ?>
							<span tabindex=-1 class="omit-row omit button" id="omit-order_<?php print $order->id; ?>">Omit</span>
						<?php endif; ?>
						<?php if (IS_EDITOR) : ?>
					...
					<?php if ($show_names) : ?>
						<td>
							<a tabindex=-1 href="<?php print site_url(format_string('common/find?genus=@genus', ['@genus' => $order->genus])); ?>" title="View all <?php print $order->genus; ?>"><?php print $order->genus; ?></a>
						</td>
					...
					<td class="order-received_midsale field">
						<?php print live_field('received_midsale', $order->received_midsale, 'order', $order->id, [
							'type' => 'text',
							'envelope' => 'span'
						]); ?>
					</td>
				...
				<?php print create_button(["text" => "Re-order", "href" => site_url("order/create?variety_id=$order->variety_id"), "id" => "oc_$order->variety_id", "class" => ["button", "new", "create", "dialog", "reorder", "order-create"], "tabindex" => "-1"]); ?>
			</td>-->
				</tr>
			<?php endforeach; ?>
		</tbody>
		<tfoot>
			<tr>
				<td colspan=<?php print $show_names ? 8 : 4; ?>></td>
				<td><?php print number_format($presale_total); ?></td>
				<td colspan=3></td>
				<td><?php print number_format($midsale_total); ?></td>
				<td colspan=5></td>
